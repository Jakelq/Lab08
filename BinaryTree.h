   ...
   BinaryTree:: size( const BinaryNode * subtree )
{
	...
	if (subtree == NULL)
		...
		treeSize = size(subtree->left_) + 1 + size(subtree->right_);
	   	return  treeSize;
}
   ...
   BinaryTree:: height( const BinaryNode * subtree )
{
	...
	{
		leftSide = height(subtree->left_);
		rightSide = height(subtree->right_);
		if (leftSide < rightSide)
		{
			treeHeight = rightSide + 1;
		}
		else
		{
			treeHeight = leftSide + 1;
		}
		return treeHeight;
	}
}
   ...
   BinaryTree:: leaves( const BinaryNode * subtree )
{
	...
	{
		leaf = leaves(subtree->left_) + leaves(subtree->right_);
		return  leaf;
	}
}
